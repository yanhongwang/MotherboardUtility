#include "KCS.h"
unsigned int state,Send,Return_value;
...
unsigned int command_port = 0xffff;
unsigned char control_code = 0xff;

void AutoCheckPort()
{
	int check1 = 0,check2 = 0;
	
	if( 0xffff == command_port )
		command_port = 0x0CA3;// it mean no command_port is specified by user
	
	if( 0xff == control_code )
		control_code = Write_Start_Code;//0x61, it mean no cmd_port_num is pecified by user
	
	outportb( command_port, control_code );
	check1 = inportb( command_port );
	...
	{
		outportb( 0x66, control_code );
		check2 = inportb( 0x66 );
		
		if( ( check2 != 0x00 ) && ( check2 != 0x1C ) && ( check2 != 0xFF ) )
		{
			KCS1_Data = 0x62;
...
void SendBMCCommand()
{
        ...
        asm cli//clear interrupt flag, clear flag "IF"
        asm push    cx
        asm push    dx
        ...
        Wait_IBF();
        asm jc      SendBMCCmdFail//jmp when carry,CF = 1
        Clear_OBF();
        asm jc      SendBMCCmdFail
        asm mov     dx, KCS1_Command
        asm mov     al, Write_Start_Code
        asm out     dx, al//SMS to BMC Write Transfer
        newiodelay();
        Wait_IBF();
        asm jc      SendBMCCmdFail
        Is_Write_State();
...
Prepare_Write_To_BMC:
        Clear_OBF();
        asm jc      SendBMCCmdFail
Write_To_BMC:// start send package to bmc
...
Yes_Write_State:
        ...
        asm jc      SendBMCCmdFail
        asm inc     si
        asm dec     cs:BMCPackageCount// count subtract one
        asm jnz     Write_To_BMC//if there is any package need to transfer,then jmp
        asm mov     dx, KCS1_Command
        asm mov     al, Write_End_Code//ending code transfer to bmc
        asm out     dx, al
        newiodelay();
        Wait_IBF();
        asm jc      SendBMCCmdFail
...
Yes_Write_State2:
        ...
        asm out     dx, al
        newiodelay();
        asm mov     cs:BMCPackageCount, 0//make package number 0
Wait_To_Read://prepare to read
        newiodelay();
        newiodelay();
        Wait_OBF();
        Is_Read_State();
        asm jc      Wait_To_Read
To_Read://change process BMC to SMS Read Transfer
        Wait_IBF();
        asm jc      SendBMCCmdFail
        Is_Read_State();
...
Yes_Idle_State:
        ...
        asm jc      SendBMCCmdFail
        asm mov     dx, KCS1_Data
        asm in      al, dx//Read dummy data byte from DATA_OUT
        newiodelay();
...
Wait_IBF_To_Zero:
        asm in      al, dx
        asm test    al, IPMI_IBF//check whether if Command or Date_In register has been written by system-side software.
        asm jz      IBF_Is_Zero//jmp when ZF=1
        newiodelay();
